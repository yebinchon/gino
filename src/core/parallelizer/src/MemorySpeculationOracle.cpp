...
static size_t split(const std::string s,
...
MemorySpeculationOracle::MemorySpeculationOracle(Noelle &noelle, Module &M)
  : DependenceAnalysis("MemorySpeculationOracle"),
    noelle(noelle) {
...
bool MemorySpeculationOracle::canThereBeAMemoryDataDependence(
  ...
  if (this->edges.count(loopid)) {
    ...
    errs() << "YEBIN: asking for profiled loop " << loopid << "\n     ";
    errs() << srcid << " . " << *src << "\n -> " << dstid << " .     " << *dst
           << "\n";
