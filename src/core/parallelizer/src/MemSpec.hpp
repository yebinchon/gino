...
struct MemorySpeculationOracle : public noelle::DependenceAnalysis {
  ...
  static bool profileExists() { return fileOpened; }

  bool canThereBeAMemoryDataDependence(Instruction *src, Instruction *dst,
                                       noelle::LoopStructure &loop);

  bool canThisDependenceBeLoopCarried(noelle::DGEdge<Value, Value> *dep,
                                      noelle::LoopStructure &loop);

  template <class T> T *getTwithId(unsigned int id, std::map<int, T *> m) {
    if (m.count(id) == 0) {
      return nullptr;
    } else {
      return m[id];
  ...
  BasicBlock *getBBWithID(unsigned int id) { return getTwithId(id, bbMap); }
